...
export default function ChatPage() {
  ...
  useEffect(() => {
    const savedSessions = localStorage.getItem("xgate_sessions");
    if (savedSessions) {
  ...
  const loadSession = (id: string) => {
    ...
    const saved = localStorage.getItem(`xgate_chat_${id}`);
  ...
  const deleteSession = (e: React.MouseEvent, id: string) => {
    ...
    setSessions(updated);
    localStorage.setItem("xgate_sessions", JSON.stringify(updated));
    localStorage.removeItem(`xgate_chat_${id}`);
  ...
  const saveChatHistory = (id: string, msgs: Message[]) => {
    localStorage.setItem(`xgate_chat_${id}`, JSON.stringify(msgs));
  };

  const updateSessionList = (id: string, titleText: string) => {
    setSessions((prev) => {
      ...
      if (existingIdx !== -1) {
        ...
        newSessions = [{ ...existing, timestamp: Date.now() }, ...others];
      } else {
        newSessions = [
          { id, title: titleText.slice(0, 30) + "...", timestamp: Date.now() },
          ...prev,
        ];
      }
      localStorage.setItem("xgate_sessions", JSON.stringify(newSessions));
      return newSessions;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentSessionId) {
      saveChatHistory(currentSessionId, newMessages);
    }
    ...
    try {
          ...
          if (line.startsWith("data:")) {
            try {
              ...
              if (parsed.session_id && !detectedSessionId) {
                ...
                updateSessionList(parsed.session_id, userMsg.content);
              }
      ...
      if (detectedSessionId) {
        const finalMessages = [...newMessages, { role: "assistant", content: fullResponse } as Message];
        saveChatHistory(detectedSessionId, finalMessages);
      }

    } catch (error) {
      console.error(error);
      setMessages((prev) => [...prev, { role: "assistant", content: "Error: Gagal terhubung ke AI." }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-1">
          {sessions.map((s) => (
            <div key={s.id} onClick={() => loadSession(s.id)} className={`group flex justify-between px-3 py-3 rounded-md cursor-pointer text-sm ${currentSessionId === s.id ? "bg-[#262626] text-white" : "text-gray-400 hover:bg-[#262626]"}`}>
              ...
              <button onClick={(e) => deleteSession(e, s.id)} className="opacity-0 group-hover:opacity-100 hover:text-red-400">
      ...
      <div className="flex-1 flex flex-col relative w-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin">
          ...
          {isLoading && !messages[messages.length - 1]?.content && (
            <div className="max-w-3xl mx-auto flex gap-4"><div className="w-8 h-8 bg-green-600 rounded-full"></div><span className="animate-pulse">Thinking...</span></div>
          )}
        ...
        <div className="p-4 bg-[#111] border-t border-gray-800">
          <div className="max-w-3xl mx-auto relative">
            {selectedImage && (
              <div className="absolute -top-16 left-0 bg-[#262626] p-2 rounded border border-gray-700 flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="relative flex gap-2">
              <label className="p-3 text-gray-400 hover:text-white cursor-pointer hover:bg-[#262626] rounded-xl transition-colors">
              ...
              <input
                ...
                placeholder={selectedImage ? "Add a caption..." : "Message XGate AI..."}
                className="w-full bg-[#262626] text-white border border-gray-700 rounded-xl pl-4 pr-12 py-3 focus:outline-none focus:border-gray-500"
                disabled={isLoading}
              />
              <button type="submit" disabled={isLoading || (!input.trim() && !selectedImage)} className="absolute right-2 top-2 p-1.5 bg-white text-black rounded-lg hover:bg-gray-200 disabled:opacity-50">
