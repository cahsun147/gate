...
ALLOWED_ORDERBY = ['buy_volume_cur', 'sell_volume_cur', 'profit', 'realized_profit', 'unrealized_profit']
...
def handler(network, contract_address, limit, orderby, direction, tag=''):
    """Menangani permintaan untuk endpoint /api/v1/tokens/traders."""
    # Validasi parameter
    if network not in ALLOWED_NETWORKS:
        return json.dumps({"error": "Jaringan tidak valid"}), 400
    if orderby not in ALLOWED_ORDERBY:
        return json.dumps({"error": "Urutan tidak valid"}), 400
    if direction not in DIRECTIONS:
        return json.dumps({"error": "Arah tidak valid, hanya 'desc' yang didukung"}), 400
    try:
        limit = int(limit)
        if limit < 1 or limit > 100:
            return json.dumps({"error": "Batas harus antara 1 dan 100"}), 400
    except ValueError:
        return json.dumps({"error": "Batas harus berupa bilangan bulat"}), 400

    # Konstruksi URL
    url = f"https://gmgn.ai/vas/api/v1/token_traders/{network}/{contract_address}"
    params = {
        'limit': limit,
        'cost': 20,
        'orderby': orderby,
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    traders = api_data.get('data', {}).get('list', [])
                    # Transformasi setiap trader untuk menghilangkan 'avatar'
                    transformed_traders = []
                    for trader in traders:
                        new_trader = {k: v for k, v in trader.items() if k != 'avatar'}
                        transformed_traders.append(new_trader)
                    # Buat struktur data baru
                    new_data = {
                        "made by": "Xdeployments",
                        "message": "ok",
                        "datatraders": {
                            "traderslist": transformed_traders
                        }
                    }
